...
If you would like to enable automated challenge and AI-enhanced project suggestions, you can connect OpenAI or any compatible service, such as the API endpoints of [OpenRouter](https://openrouter.ai) or the [LiteLLM](https://docs.litellm.ai/) proxy.

* `LLM_BASE_URL` - the address of your endpoint, e.g. <https://api.publicai.co>
...
* `LLM_TITLE` - the name of the model that is visible to users (if blank, `LLM_MODEL` is shown)

When this is configured, three additional features will appear in the UI:

* An option to generate a challenge pitch based on a title and summary
* AI coaching in the Posts area of a project (this is the one most visible to teams)
* Admins are able to add an Evaluation to the Log of a project (same screen as above)

All content generated by LLMs is clearly marked in the interface with disclaimers like these:

`🅰️ℹ️ generated with APERTUS-70B`

_We welcome your feedback on this feature, and any questions you have about the use of AI at your hackathon._
